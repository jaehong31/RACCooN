...
from ..model.video_diffusion.unet import VideoInpaintingModel
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    state_dict = trainer.model.unet.state_dict() # save checking here
...
# def load_mask(video_path, indices, mask_id):
#     WIDTH = 512
#     HEIGHT = 320
...
#         m = (all_mask == int(mask_id)).astype(np.uint8) * 255
#         image = Image.fromarray(m)
#         image = image.resize((WIDTH, HEIGHT), resample=Image.BILINEAR)
#         frames.append(image)

#     frames = np.stack(frames, axis=2)
#     # print('mask', frames.shape)
...
#     # out = torch.nonzero(frames)
#     # out = torch.unique(frames)
#     # print(out)
#     return frames
...
def load_video(video_path, sample_num=16, sample_type='random', given_index=None):
    WIDTH = 512
    HEIGHT = 320
...
class LazySupervisedDataset_Video(Dataset):
    ...
    def __init__(
                ...
                #  ,tokenizer: transformers.PreTrainedTokenizer,
                #  multimodal_cfg: dict):
        super(LazySupervisedDataset_Video, self).__init__()

        # data_meta = '/nas-ssd2/jhyoon/vl_codebook/models/Video-LLaVA/gemini_instruction_gen_outputs/instgen_lgvi_gt_0.json'
    ...
    #     pixel_values, index = load_video(os.path.join(self.video_base_path, vid), sample_num=self.frame_num)
    #     mask = load_mask(os.path.join(self.mask_base_path, vid), index, mask_id)
        
    #     data_dict = {}
    #     data_dict['video'] = pixel_values # [1, 8, 3, 320, 512]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'target' in instances[0]:
            target = [instance['target'] for instance in instances]
            if all(x is not None and x.shape == target[0].shape for x in target):
                batch['target'] = torch.stack(target)
            else:
                batch['target'] = target
                
        if 'condition' in instances[0]:
            condition = [instance['condition'] for instance in instances]
            if all(x is not None and x.shape == condition[0].shape for x in condition):
                batch['condition'] = torch.stack(condition)
            else:
                batch['condition'] = condition
        
        if 'mask' in instances[0]:
            mask = [instance['mask'] for instance in instances]
            if all(x is not None and x.shape == mask[0].shape for x in mask):
                batch['mask'] = torch.stack(mask)
            else:
                batch['mask'] = mask
            
        if 'task' in instances[0]:
            task = [instance['task'] for instance in instances]
            batch['task'] = task
                
        if 'text_prompt' in instances[0]:
            prompt = [instance['text_prompt'] for instance in instances]
            batch['prompt'] = prompt
...
def train():
    ...
    print(model_args)
    # ModelArguments(model_name_or_path='/nas-hdd/shoubin/pretrained_model/mgie_ckpt/LLaVA-Lightning-7B-delta-v1-1', version='v1', freeze_backbone=False, tune_mm_mlp_adapter=False, vision_tower='openai/clip-vit-large-patch14', mm_vision_select_layer=-2, pretrain_mm_mlp_adapter=None, mm_use_im_start_end=True)
    print(data_args)
    # DataArguments(data_path=None, lazy_preprocess=True, is_multimodal=False, sep_image_conv_front=False, image_token_len=0, image_folder=None, image_aspect_ratio='square')
    print(training_args)
    # 
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            print('here we load the model')
            model = LlavaLlamaForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                **bnb_model_from_pretrained_args
            )
            model.unet = VideoInpaintingModel.from_pretrained('/nas-hdd/shoubin/pretrained_model/lgvi/lgvi', subfolder='unet')
    else:
    ...
    pt = {}
    # for i in tqdm(range(2)): pt.update(torch.load('/nas-hdd/shoubin/pretrained_model/mgie_ckpt/LLaVA-Lightning-7B-delta-v1-1/pytorch_model-0000%d-of-00002.bin'%(i+1), map_location='cpu'))
