# modified from https://github.com/apple/ml-mgie/blob/main/mgie_train.py
...
from llava.train.llava_trainer import RACCooNTrainer
...
@dataclass
class RACCooNArguments:
    RACCooN: bool = field(default=False)
    ...
    for frame_name in selected_frames:
        ...
        if convert_to_box:
            ...
            try:
                ...
                if normalize:
                    box = '%d, %d, %d, %d'%(xmin/WIDTH*1000, ymin/HEIGHT*1000, xmax/WIDTH*1000, ymax/HEIGHT*1000)
                else:
        ...
        for gtf in gt_files:
            ...
            with open(gt_path, "rb") as f:
                ...
                if not 'error' in gt_response.keys():
                    ...
                    if pre_resp != '':
                        self.gt_lists.append(pre_resp)
                        self.gt_query_type.append(0)
                        self.video_file_names.append(gtf.split(".")[0])

        print(f'gt_lists len: {len(self.gt_lists)}')
...
def train(task='multi-obj'):
    parser = transformers.HfArgumentParser((ModelArguments, DataArguments, TrainingArguments, RACCooNArguments))
    model_args, data_args, training_args, raccoon_args = parser.parse_args_into_dataclasses()
    ...
    trainer = RACCooNTrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
...
if __name__ == "__main__":
    ...
    train(task='multi-obj')
    train(task='single-obj')
    train(task='layout-pred')
